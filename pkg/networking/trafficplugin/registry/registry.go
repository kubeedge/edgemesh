...
func (esd *EdgeServiceDiscovery) FindMicroServiceInstances(consumerID, microServiceName string, tags utiltags.Tags) ([]*registry.MicroServiceInstance, error) {
	...
	klog.Infof("FindMicroServiceInstances::eps:%s --> address:%s -> port:%d", eps.Name, eps.Subsets[0].Addresses[0].IP, eps.Subsets[0].Ports[0].Port)
	// get pods
	...
	// port not found
	if targetPort == 0 {
		klog.Errorf("[EdgeMesh] port %d not found in svc: %s.%s", svcPort, namespace, name)
