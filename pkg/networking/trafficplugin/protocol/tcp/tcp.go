...
func (p *TCP) Process() {
	...
	klog.V(4).Infof("MicroServiceName:%s, Endpoint:%s, Port:%s, SourceServiceID:%s, Protocol:%s, Strategy:%s, Args:%v", inv.MicroServiceName, inv.Endpoint, inv.Port, inv.SourceServiceID, inv.Protocol, inv.Strategy, inv.Args)
	// create handlerchain
...
func (p *TCP) responseCallback(data *invocation.Response) error {
	...
	klog.V(4).Infof("responseCallback:ep:%s", ep)
...
func (c *conntrack) processClientProxy() {
	buf := make([]byte, config.Config.Protocol.TCPBufferSize)
	for {
